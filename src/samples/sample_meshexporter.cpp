...
/* This sample (which is actually used code by me) exports all tiles of a map
 * with a little border around it, its x/y-position, the bounding box (border
 * excluded!), vertices and indices. The meshes perfectly fit further
...
int main(int argc, char **argv) {
  ...
  for (AdtPos_t::iterator pos = adt_pos.begin();
    ...
    if (GetTile(map, border_min, border_max, &mesh)) {
      ...
      fs.write((char*)mesh.indices().data(), idx_mem_size);
      ...
      fs.write((char*)mesh.vertices().data(), vtx_mem_size);
...
void LoadAdts(const std::string &dir, const std::string &wdtDir,
  ...
  // start loading
  for (AdtPos_t::const_iterator pos = pos_begin; pos != pos_end; ++pos, count++) {
    Meshes_t meshes;
