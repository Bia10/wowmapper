...
int32_t libmpq__archive_open(mpq_archive_s **mpq_archive, const char *mpq_filename, libmpq__off_t archive_offset) {
  ...
  if ((*mpq_archive = (mpq_archive_s*)calloc(1, sizeof(mpq_archive_s))) == NULL) {
	...
	while (TRUE) {
		...
		/* seek in file. */
		if (libmpq__fseek((*mpq_archive)->fp, archive_offset, SEEK_SET) < 0) {
	...
	if ((*mpq_archive)->mpq_header.version == LIBMPQ_ARCHIVE_VERSION_TWO) {
		...
		/* seek in file. */
		if (libmpq__fseek((*mpq_archive)->fp, sizeof(mpq_header_s) + archive_offset, SEEK_SET) < 0) {
	...
	if (((*mpq_archive)->mpq_block    = (mpq_block_s*)calloc((*mpq_archive)->mpq_header.block_table_count, sizeof(mpq_block_s))) == NULL ||
	    ((*mpq_archive)->mpq_block_ex = (mpq_block_ex_s*)calloc((*mpq_archive)->mpq_header.block_table_count, sizeof(mpq_block_ex_s))) == NULL ||
	    ((*mpq_archive)->mpq_hash     = (mpq_hash_s*)calloc((*mpq_archive)->mpq_header.hash_table_count,  sizeof(mpq_hash_s))) == NULL ||
	    ((*mpq_archive)->mpq_file     = (mpq_file_s**)calloc((*mpq_archive)->mpq_header.block_table_count, sizeof(mpq_file_s))) == NULL ||
	    ((*mpq_archive)->mpq_map      = (mpq_map_s*)calloc((*mpq_archive)->mpq_header.block_table_count, sizeof(mpq_map_s))) == NULL) {
	...
	/* seek in file. */
	if (libmpq__fseek((*mpq_archive)->fp, (*mpq_archive)->mpq_header.hash_table_offset + (((long long)((*mpq_archive)->mpq_header_ex.hash_table_offset_high)) << 32) + (*mpq_archive)->archive_offset, SEEK_SET) < 0) {
	...
	/* seek in file. */
	if (libmpq__fseek((*mpq_archive)->fp, (*mpq_archive)->mpq_header.block_table_offset + (((long long)((*mpq_archive)->mpq_header_ex.block_table_offset_high)) << 32) + (*mpq_archive)->archive_offset, SEEK_SET) < 0) {
	...
	if ((*mpq_archive)->mpq_header_ex.extended_offset > 0) {
		...
		/* seek in file. */
		if (libmpq__fseek((*mpq_archive)->fp, (*mpq_archive)->mpq_header_ex.extended_offset + archive_offset, SEEK_SET) < 0) {
...
int32_t libmpq__block_open_offset(mpq_archive_s *mpq_archive, uint32_t file_number) {
	...
	if ((mpq_archive->mpq_block[mpq_archive->mpq_map[file_number].block_table_indices].flags & LIBMPQ_FLAG_COMPRESSED) != 0 &&
		...
		if (libmpq__fseek(mpq_archive->fp, mpq_archive->mpq_block[mpq_archive->mpq_map[file_number].block_table_indices].offset + (((long long)mpq_archive->mpq_block_ex[mpq_archive->mpq_map[file_number].block_table_indices].offset_high) << 32) + mpq_archive->archive_offset, SEEK_SET) < 0) {
...
int32_t libmpq__block_read(mpq_archive_s *mpq_archive, uint32_t file_number, uint32_t block_number, uint8_t *out_buf, libmpq__off_t out_size, libmpq__off_t *transferred) {
	...
	/* seek in file. */
	if (libmpq__fseek(mpq_archive->fp, block_offset + mpq_archive->archive_offset, SEEK_SET) < 0) {
